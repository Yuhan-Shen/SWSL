...
from mucon.restricted_editdistance_loss import SoftRED_Loss
from .flexible_transcript_pred import FTP_Layer
...
def create_semi_supervision_model(
    cfg: CfgNode,
    num_classes: int,
    max_decoding_steps: int,
    input_feature_size: int,
) -> "MuConSemiSupervision":
    model_name = cfg.model.name
    if model_name == "mucon":
        return MuConSemiSupervision(
            cfg=cfg,
            input_feature_size=input_feature_size,
            num_classes=num_classes,
            max_decoding_steps=max_decoding_steps,
...
class MuCon(Model):
    def __init__(
        ...
        # define a soft restricted edit distance loss function
        self.sre_loss_fn = SoftRED_Loss(
                alpha=self.cfg.add_params.sre.alpha, 
                center_norm=self.cfg.add_params.sre.center_norm,
                sim=self.cfg.add_params.sre.sim,
                threshold=self.cfg.add_params.sre.threshold,
                swap_threshold=self.cfg.add_params.sre.swap_threshold,
                softmax=self.cfg.add_params.sre.softmax,
                norm_by_len=self.cfg.add_params.sre.norm_by_len,
                )

        self.pooling_stride = self.cfg.add_params.ftp.pooling_stride
        self.n_seg_ratio = self.cfg.add_params.ftp.n_seg_ratio

    def ftp(self, batch, forward_out, avg_n_steps, given_actions=None):
        ftp_input = F.log_softmax(forward_out.segmentation, dim=-1)
        if self.pooling_stride > 1:
            ftp_input = F.avg_pool1d(ftp_input.t().unsqueeze(0), self.pooling_stride, self.pooling_stride).squeeze(0).t()
        if batch.is_weakly:
            raise NotImplementedError
        else:
            # use a ftp layer to get the flexible transcripts (prototypes) of an unlabeled video
            ftp_layer = FTP_Layer(
                    min_n_seg=max(3, round(avg_n_steps * self.n_seg_ratio[0])), # the minimum n_seg is 3
                    max_n_seg=max(3, round(avg_n_steps * self.n_seg_ratio[1])), # the minimum n_seg is 3
                    penalty_weight=self.cfg.add_params.ftp.penalty_weight,
                    use_avg_prob=self.cfg.add_params.ftp.use_avg_prob,
                    )
        prototypes = ftp_layer(ftp_input, given_actions)
        return prototypes

    def ftp_loss(self, batch: Batch, forward_out: MuConForwardOut, transcripts_per_task: dict = None, actions_per_task: dict = None, avg_n_steps_per_task: dict=None) -> Tensor:
        # compute the ftp loss, i.e. the SRE loss between each weak-labeled transcript and the predicted transcript by FTP
        if self.cfg.add_params.ftp.given_actions_train:
            possible_actions = actions_per_task[batch.task_label.item()]
            prototypes = self.ftp(batch, forward_out, 
                    avg_n_steps=avg_n_steps_per_task[batch.task_label.item()],
                    given_actions=torch.tensor(list(possible_actions)))
        else:
            prototypes = self.ftp(batch, forward_out,
                    avg_n_steps=avg_n_steps_per_task[batch.task_label.item()])

        possible_target_transcripts = transcripts_per_task[batch.task_label.item()]
        loss_list = []
        for transcript in possible_target_transcripts:
            transcript_tf_target = torch.tensor(
                transcript).long().cuda()
            transcript_onehot = F.one_hot(transcript_tf_target, self.EOS_token_id).float()
            loss = self.sre_loss_fn(transcript_onehot, prototypes)
            loss_list.append(loss)

        # use the minimum loss or average loss
        if self.cfg.add_params.ftp.match_reduction == 'min':
            return min(loss_list)
        elif self.cfg.add_params.ftp.match_reduction == 'mean':
...
class MuConSemiSupervision(MuCon):
    def forward(self, batch: SemiSupervisionBatch) -> MuConForwardOut:
        if self.training and batch.weakly_labeled:
            self.set_teacher_forcing(teacher_forcing=True)
        else:
            self.set_teacher_forcing(teacher_forcing=False)
        return super().forward(batch)

    def transcript_loss(self, batch: SemiSupervisionBatch, forward_out: MuConForwardOut) -> Tensor:
        if self.training and batch.weakly_labeled:
            return super().transcript_loss(batch, forward_out)
        else:
